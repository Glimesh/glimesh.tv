  ...
  describe "authorize oauth2" do
    ...
    test "GET /oauth/authorize", %{conn: conn, oauth_app: app} do
      conn =
        get(
          conn,
          Routes.authorization_path(conn, :new,
      ...
      assert html_response(conn, 200) =~
               "Are you sure you wish to authorize some name to use your account? This application will be able to:"
    end
    ...
    test "POST /oauth/authorize deny", %{conn: conn, oauth_app: app} do
      ...
      assert html_resp =~
               "http://localhost:8080/redirect?error=access_denied&amp;error_description=The+resource+owner+or+authorization+server+denied+the+request."
      ...
      conn =
        post(
          ...
          Routes.authorization_path(conn, :create,
            ...
            client_secret: app.client.secret,
            scopes: "public",
            redirect_uri: URI.encode("urn:ietf:wg:oauth:2.0:oob"),
            response_type: "code",
            action: "authorize"
          )
        )
