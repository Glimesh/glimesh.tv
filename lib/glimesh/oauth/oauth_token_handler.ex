defmodule Glimesh.OauthHandler.Token do
  @moduledoc false
  ...
  defp issue_grant(
         {:ok,
          %{resource_owner: resource_owner, client: _application, request: request} = params},
         config
       ) do
  ...
  def authorize_response({:ok, params}, config),
    do:
      build_response(
        params,
        %{
          access_token: params.access_token.token,
          token_type: "bearer",
          expires_in: params.access_token.expires_in,
          scope: params.access_token.scopes
        },
        config
      )

  def authorize_response({:error, %{error: error} = params}, config),
    do: build_response(params, error, config)
