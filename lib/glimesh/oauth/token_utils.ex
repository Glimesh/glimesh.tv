defmodule Glimesh.OauthHandler.TokenUtils do
  ...
  def load_client_introspec({:ok, params}, _config),
    do: OauthError.add_error({:ok, params}, OauthError.invalid_ownership())
  ...
  def load_access_token({:ok, params}, _config),
    do: Error.add_error({:ok, params}, Error.invalid_request())
  ...
  def load_resource_owner({:ok, %{access_token: access_token} = params}, config) do
    {:ok,
     Map.put(params, :access_token, Config.repo(config).preload(access_token, :resource_owner))}
  end
end
