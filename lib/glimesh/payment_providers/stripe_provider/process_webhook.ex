defmodule Glimesh.PaymentProviders.StripeProvider.ProcessWebhook do
  ...
  defp process_event(type, data) do
    case [type, data] do
      ...
      # Regular methods like credit cards
      ...
      # Async methods like SEPA Direct Debit
      ["checkout.session.async_payment_succeeded", %{object: %Stripe.Session{} = session}] ->
        if session.payment_status == "paid" do
          StripeProvider.complete_session(session)
        end

      # TODO: Still needed since we've moved to checkout sessions entirely?
