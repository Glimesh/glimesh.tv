defmodule Glimesh.Interactive do
  ...
  def validate({%Waffle.File{} = file, _channel}) do
    with true <- file_size(file) <= @max_file_size, # check file size
      {:ext_check, true} <- {:ext_check, String.ends_with?(to_string(file.file_name), @allowed_files)} do
        :ok
      else
  ...
  def filename(:original, {file, %Glimesh.Streams.Channel{} = _channel}) do
    ...
    #Remove the file extension since ecto will add that. Global isn't needed as the file names have already been filtered to be acceptable
