defmodule GlimeshWeb.SupportModal.SubForm do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <%= if @user do %>
        <form id="subscription-form" phx-target={@myself} phx-submit="submit">
          <p>
  ...
  def handle_event("submit", _, socket) do
    ...
    return_url = url(~p"/#{streamer.username}/support/subscribe")

    amount = Glimesh.Payments.get_channel_sub_base_price()

    case Glimesh.Payments.start_subscription(
           user,
           streamer,
           amount,
           return_url
