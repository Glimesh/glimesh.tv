defmodule GlimeshWeb.UserLive.Components.StreamerTitle do
  ...
  def mount(_params, %{"streamer" => streamer, "user" => nil}, socket) do
    if connected?(socket), do: Glimesh.Streams.subscribe_metadata(streamer.id)
    {:ok,
     socket
  ...
  def mount(_params, %{"streamer" => streamer, "user" => user}, socket) do
    ...
    {:ok,
     ...
     |> assign(:is_streamer, user.username == streamer.username)
     |> assign(:editing, false)}
  end
  ...
  def handle_event("save", %{"stream_metadata" => %{"stream_title" => new_title}}, socket) do
    case Glimesh.Streams.update_title(socket.assigns.streamer, new_title) do
      {:ok, changetitle} ->
        {:noreply,
        socket
        |> assign(:editing, false)
  ...
  def handle_info({:update_title, data}, socket) do
    cond do
      data.streamer.username == socket.assigns.streamer.username -> {:noreply, assign(socket, title: data.title)}
      true -> {:noreply, socket}
    end
  end
