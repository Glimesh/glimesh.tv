defmodule GlimeshWeb.Channels.DebugModalComponent do
  ...
  @impl Phoenix.LiveView
  def handle_info({:debug, :packet_loss, lost_packets}, socket) when is_integer(lost_packets) do
    message =
      if lost_packets > 6,
        do:
          gettext(
  ...
  def handle_info({:debug, :janus_info, %{janus_hostname: hostname, janus_url: url}}, socket) do
    {:noreply, assign(socket, janus_hostname: hostname, janus_url: url)}
  end
