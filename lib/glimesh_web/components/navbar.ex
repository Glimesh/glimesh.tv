            ...
            <div class="hidden sm:ml-6 sm:block">
              <div class="flex space-x-2">
                <div class="group inline-block relative">
                  <.nav_link
                    navigate={Routes.streams_index_path(@conn, :index)}
                    class="rounded-lg group-hover:bg-gray-700 group-hover:rounded-b-none"
                  >
                    <%= gettext("Browse") %>
                  </.nav_link>

                  <div class="absolute hidden rounded-lg rounded-tl-none bg-gray-700 z-50 pt-1 group-hover:block">
                    <div class="flex justify-between">
                      <%= for {name, category, icon} <- list_categories() do %>
                        <.link
                          navigate={Routes.streams_index_path(GlimeshWeb.Endpoint, :index, category)}
                ...
                <%= if assigns[:current_user] do %>
                  <.nav_link navigate={Routes.streams_index_path(@conn, :index, "following")}>
                    <%= gettext("Following") %>
                    <%= cond do %>
                ...
                <% end %>
                <.nav_link navigate={Routes.events_path(@conn, :index)}>
                  <%= gettext("Events") %>
                </.nav_link>
              </div>
            </div>
